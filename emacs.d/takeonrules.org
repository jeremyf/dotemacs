...
#+begin_src emacs-lisp
  ...
  This prevents links to non-world accessible files."
    (let* ((node (org-roam-node-from-id (org-element-property :path link))))
      (or (jf/org-md-render-link-for :node node :desc desc)
	  (format "<span class=\"ref\">%s</span>" desc))))

  (cl-defun jf/org-md-render-link-for (&key node desc)
  ...
  ;;; For testing:
  ;;
  ;; (message "%s" (jf/org-md-render-link-for :node (org-roam-node-from-id "FC017488-D8EC-43DE-A35D-4D10A87B6A0D") :desc "Burning Wheel Gold"))
  ;; (message "%s" (jf/org-md-render-link-for :node (org-roam-node-from-id "86F3E44F-AA0E-4B08-B0D8-30A764B4CD13") :desc "Org Roam"))
#+end_src
