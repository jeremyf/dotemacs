...
#+begin_src emacs-lisp
  (use-package org
    ...
    (cl-defun jf/org-mode-find-project-node ()
	...
	(let* ((title (jf/org-mode-project-prompt))
	       (position (org-element-map (org-element-parse-buffer) 'headline
			       (lambda (hl)
				 (and (= (org-element-property :level hl) 4)
				      (member "project" (org-element-property :tags hl))
				      (string= title (plist-get (cadr hl) :raw-value))
