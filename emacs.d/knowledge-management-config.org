...
#+begin_src emacs-lisp
  ...
  (cl-defun jf/org-goto-clock-or-agenda ()
    "Goto the current clock if one exists; otherwise today's agenda heading."
    (interactive)
    (require 'pulsar)
	...
	(beginning-of-buffer)
	(search-forward (format-time-string "%Y-%m-%d %A"))
...
#+begin_src emacs-lisp
  (use-package org
    ...
    :bind ("C-j" . jf/org-goto-clock-or-agenda)
    :bind (:map org-mode-map ("C-j" . jf/org-goto-clock-or-agenda))
    :config (setq
...
#+begin_src emacs-lisp
  (cl-defun jf/org-mode-agenda-to-stand-up-summary (&key (within_headline (format-time-string "%Y-%m-%d %A")))
