	...
	(let* ((properties (org-roam-node-properties node))
	  ...
	  (pcase format
	    (`html (if use_hugo_shortcode
		     ...
		     (if-let (url (jf/org-roam-external-url-for :node node))
			 (format "<cite><a href=\"%s\">%s</a></cite>" url title)
...
#+begin_src emacs-lisp
  (cl-defun jf/org-roam-external-url-for (&key node)
    "Return an external URL for the given NODE.

  Note, I use the `org-roam-node-properties' instead of
  `org-roam-node-refs' because the latter strips the scheme."
    (seq-find (lambda (el) (s-starts-with? "http" el))
	      (split-string (format "%s" (alist-get "ROAM_REFS"
