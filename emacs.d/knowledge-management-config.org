...
#+begin_src emacs-lisp
  ...
  (cl-defun jf/org-goto-clock-or-agenda ()
    "Goto the current clock if one exists; otherwise the agenda file"
    (interactive)
...
#+begin_src emacs-lisp
  (use-package org
    ...
    (setq org-capture-templates
	  '(("@"
	    ...
	    ("l" "Learning Time"
	     ...
	     "* Learning Time   :nonbillable: \n%?"
	     :empty-lines-before 1
	     :clock-in 1)
	    ("t" "Timesheet"
	     entry (file+olp+datetree jf/primary-agenda-filename-for-machine)
	     "* %?   :%^{PROMPT|billable|nonbillable}:"
	     :empty-lines-before 1
	     :clock-in 1)
	    ("p" "Project"
	     entry (file+olp+datetree jf/primary-agenda-filename-for-machine)
    ...
    (cl-defun jf/org-mode-find-project-node (&key (project (jf/org-mode-project-prompt)))
      "Find and position the cursor at the end of the given PROJECT."
      (with-current-buffer (find-file-noselect jf/primary-agenda-filename-for-machine)
	(goto-char (or (org-element-map (org-element-parse-buffer) 'headline
			       (lambda (hl)
				 (and (= (org-element-property :level hl) 4)
				      (member "project" (org-element-property :tags hl))
				      ;; I can't use the :title attribute as it's
  ...
  \\hypersetup{pdfborder=0 0 0}"
    ...
    (org-clock-clocktable-default-properties '(:maxlevel 4 :link t :tags t))
...
#+RESULTS:
: /opt/homebrew/Cellar/plantuml/1.2022.6/libexec/plantuml.jar
