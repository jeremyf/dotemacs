...
I grabbed from the following LaTeX class from https://www.reddit.com/r/emacs/comments/3zcr43/nooborgmode_custom_latexpdf_export_custom_style/.  I’m trash with LaTeX, but like the layout thusfar.
...
#+begin_src emacs-lisp
  ...
  Filter selections to INITIAL-INPUT."
    (interactive)
    (let ((node (org-roam-node-read initial-input (lambda (node) (jf/org-roam-filter-context-fn node :tag-list '("epigraphs"))))))
      (insert (format "\n{{{epigraph(%s,%s)}}}\n"
