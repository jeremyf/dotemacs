...
The following function tracks to the following feature request: [[https://github.com/org-roam/org-roam/issues/2119][Allow for `org-roam-buffer`'s Backlinks section to be unique per source · Issue #2119 · org-roam/org-roam]].

/Note: I’ve chosen to overwrite the org-roam function./  Without this function, given my org-roam reference strategy, I have a tremendous number of backlinks.

#+begin_src emacs-lisp
  (defun org-roam-backlinks-get (node)
    "Return the backlinks for NODE.  Limit to unique sources."
    (let ((backlinks (org-roam-db-query
		      [:select :distinct [source dest (funcall min pos) (funcall min properties)]
		       :from links
		       :where (= dest $s1)
		       :and (= type "id")
		       :group :by source]
