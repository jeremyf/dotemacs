...
(pretty-hydra-define jnf/tor-subject-menu-markdown (:foreign-keys warn :title jnf/tor-menu--title :quit-key "q" :exit t)
  ("Wrapping"
   (("w a" jnf/tor-wrap-link-active-region-dwim "A link at point or region…")
    ("w c" jnf/tor-wrap-cite-active-region-dwim "Cite point or region…")
    ("w d" jnf/tor-wrap-date "Date point or region…")
    ("w f" jnf/tor-wrap-as-pseudo-dfn "Wrap word or region in pseudo-dfn…")
   ...
   (("c a" jnf/tor-post-amplifying-the-blogosphere "Create amplify the blogosphere…")
    ...
    ("t s" (jnf/tor-toggle-hugo-server :buffer-name "*Hugo Server*") "Toggle hugo server" :toggle (get-buffer "*Hugo Server*")))))
